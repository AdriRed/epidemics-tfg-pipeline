...
- Estados: susceptibles (blanco), infectados (rojo), recuperados (azul).

Uso:
    python anim_epidemia_opt.py coords.dat edges.dat events.dat output_frames/ \\
...
# =============================================================================
# (No hay configuración específica para este script, pero se puede añadir)

# =============================================================================
# 1. FUNCIONES HIPERBÓLICAS (para leer datos y calcular distancias)
# =============================================================================
...
def read_hyperbolic_data(archivo_coords: str, archivo_edges: str):
    ...
    Además, calcula la distancia hiperbólica para cada arista si no existe.
    """
    ...
    # Calcular distancias hiperbólicas para las aristas si no existen
    for u, v in G.edges():
...
def radial_layout_brockmann_helbing(G, root, weight='distance', angle_span=2*np.pi, scale=0.9):
    ...
    """
    # 1. Calcular capas BFS (profundidad topológica) y distancia acumulada (Dijkstra)
    layers = {root: 0}
    accumulated_dist = {root: 0.0}
    queue = [root]
    ...
    for node in queue:
        for neighbor in G.neighbors(node):
            if neighbor not in layers:
                ...
                # Distancia acumulada desde la raíz = distancia del padre + peso de la arista
                accumulated_dist[neighbor] = accumulated_dist[node] + G[node][neighbor][weight]
                queue.append(neighbor)

    # Para nodos no alcanzables (por si acaso)
    for node in G.nodes():
        if node not in layers:
            layers[node] = -1
            accumulated_dist[node] = 0.0

    # 2. Agrupar nodos por capa
    nodes_by_layer = {}
    for node, layer in layers.items():
        if layer >= 0:  # ignorar nodos no alcanzables
            nodes_by_layer.setdefault(layer, []).append(node)

    # 3. Asignar radios (distancias radiales) normalizadas al intervalo [0, 1]
    max_dist = max(accumulated_dist.values()) if accumulated_dist else 1.0
    if max_dist == 0:
        max_dist = 1.0
    ...
    for node in G.nodes():
        if node == root:
            ...
            radii[node] = (accumulated_dist[node] / max_dist) * scale
...
# =============================================================================
# 4. RENDERIZADO DE UN FRAME (con layout radial y tres estados)
# =============================================================================
def render_frame_radial_three_states(ax, susceptible_coords, infected_coords, recovered_coords, time=None):
    """Dibuja nodos según estado: susc=blanco, inf=rojo, rec=azul."""
    def unpack(coords):
        if len(coords) > 0:
            return zip(*coords)
        else:
    ...
    # Infectados (rojo)
    if len(x_inf) > 0:
        ax.scatter(x_inf, y_inf, s=20, c='red', edgecolors='darkred')
    # Recuperados (azul)
    if len(x_rec) > 0:
        ax.scatter(x_rec, y_rec, s=20, alpha=0.3, c='blue', edgecolors='darkblue')
...
def mercator_epidemic_disc_radial_three_states(df, susceptible_coords, infected_coords, recovered_coords,
                                               filename=None, time=None):
    """Versión del renderizado que usa el layout radial y tres estados."""
    fig = Figure(dpi=200, figsize=(5, 5))
    ax = fig.add_subplot(111)
    render_frame_radial_three_states(ax, susceptible_coords, infected_coords, recovered_coords, time)
    fig.tight_layout()
    if filename:
...
def generate_epidemic_frames(df, G: nx.Graph, events, output_dir, step=0.1, t_start=None, t_end=None,
    ...
    if parallel:
        ...
        def render_frame(args):
            ...
            mercator_epidemic_disc_radial_three_states(
                df, susc, inf, rec,
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="Generador de frames de epidemia con layout radial (3 estados)")
    parser.add_argument("coords_file", help="Archivo de coordenadas (inf_coord o gen_coord)")
    parser.add_argument("edges_file", help="Archivo de aristas (para leer parámetros y distancias)")
